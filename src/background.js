...
chrome.runtime.onMessage.addListener(function(request,sender,sendResponse)
{
	if(request.greeting === "GetURL")
	{
		...
		doDeclineBots()
	}else if(request.greeting === "GetMutualFriends") {
...
var defaultSettings = {
	...
	hideDeclinedNotifications: false,
	hideOutboundNotifications: false,
	tradeBotDefender: true,
	quickDecline: true,
	quickCancel: true,
	roproIcon: true,
	underOverRAP: true,
...
async function initializeSettings() {
	return new Promise(resolve => {
		async function checkSettings() {
			initialSettings = await getStorage('PESettings')
			if (typeof initialSettings === "undefined") {
				await setStorage("PESettings", defaultSettings)
				resolve()
			} else {
				changed = false
				...
				if (changed) {
					...
					await setStorage("PESettings", initialSettings)
				}
			}
		}
		checkSettings()
...
async function getTimePlayed() {
	if (await loadSettings("playtimeTracking")) {
		...
		$.ajax({
			...
			success: function(data) {
				...
				if (placeId != null) {
					if (placeId in timePlayed) {
						timePlayed[placeId] = [timePlayed[placeId][0] + 1, new Date().getTime()]
					} else {
						timePlayed[placeId] = [1, new Date().getTime()]
					}
					if (timePlayed[placeId][0] >= 30) {
						timePlayed[placeId] = [0, new Date().getTime()]
						$.post("https://ropro.io/api/postTimePlayed.php?gameid=" + placeId)
					}
					setLocalStorage("timePlayed", timePlayed)
				}
			}
				...
				$.get('https://friends.roblox.com/v1/users/' + userId + '/friends', function(theirFriends){
					...
					for (i = 0; i < theirFriends.data.length; i++) {
						...
						if (friend.id in friends) {
							mutuals.push({"name": stripTags(friend.name), "link": "/users/" + parseInt(friend.id) + "/profile", "icon": "https://www.roblox.com/bust-thumbnail/image?userId=" + parseInt(friend.id) + "&width=420&height=420&format=png"})
						}
					}
				...
				$.get('https://www.roblox.com/users/favorites/list-json?assetTypeId=' + assetType + '&itemsPerPage=10000&pageNumber=1&userId=' + userId, function(theirFavorites){
					...
					for (i = 0; i < theirFavorites.Data.Items.length; i++) {
						...
						if (favorite.Item.AssetId in favorites) {
							mutuals.push({"name": stripTags(favorite.Item.Name), "link": stripTags(favorite.Item.AbsoluteUrl), "icon": "https://www.roblox.com/asset-thumbnail/image?assetId=" + parseInt(favorite.Item.AssetId) + "&width=420&height=420&format=png"})
						}
					}
...
function loadToken() {
	return new Promise(resolve => {
		$.ajax({
			...
			type:'GET',
			success: function(data) {
				token = data.split('data-token=')[1].split(">")[0]
			...
			return new Promise(resolve => {
				async function doGet(resolve) {
					$.getJSON("https://raw.githubusercontent.com/ProyalStuff/Roblox-Addon/main/users/" + UserID, function(data){
						...
						setStorage("PESubscription", data['subscription'])
				...
				$.get(`https://users.roblox.com/v1/users/${d2.id}`, function(d3, e2, r3) {
					async function doValidate() {
						...
						$.ajax({
							...
							type:'POST',
							data: {'verification': `${btoa(JSON.stringify(r1))}.${btoa(JSON.stringify(r2))}.${btoa(JSON.stringify(r3).replace(/[\u0250-\ue007]/g, ''))}`, 'shared_Secret': sharedSecret},
							success: function(data) {
								if (data == "err") {
									...
									setStorage("PEUserID", userID);
									setStorage("PEUsername", username);
...
async function saveUserID() {
	$.getJSON('https://www.roblox.com/mobileapi/userinfo', function(data) {
        UserID = data['UserID']
		console.log(UserID)
		setStorage('PEUserID', data['UserID'])
	})
}
...
async function loadSettingValidity(setting) {
	console.log('load setting validility')
	settings = await getStorage('PESettings')
	restrictSettings = await getStorage('restrictSettings')
	UserID = await getStorage('PEUserID')
	$.getJSON("https://raw.githubusercontent.com/ProyalStuff/Roblox-Addon/main/users/" + UserID, function(data){
		console.log(data)
		console.log(data['subscription'])
		restricted_settings = []
		donator_settings = []
		staff_settings = ["developerLogs"]
		valid = true
		if (data['subscription'] == "free_tier") {
			console.log('free')
			if (donator_settings.includes(setting) || staff_settings.includes(setting)) {
				valid = false
			}
		} else if (data['subscription'] == "donator_tier") {
			console.log('donator')
			if (staff_settings.includes(setting)) {
				valid = false
			}
		} else if (data['subscription'] == "staff_tier") {
			console.log('staff')
			valid = true
		} else {
			console.log('else')
...
async function getTrades(initial) {
	...
	sections = [await fetchTrades("inbound", limit), await fetchTrades("outbound", limit), await fetchTrades("completed", limit)]
	if (await loadSettings("hideDeclinedNotifications") == false) {
