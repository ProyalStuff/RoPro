...
async function addUserTier() {
	if (await fetchSetting("profileTier")) {
		if (location.href.includes("/profile")) {
			...
			$.getJSON("https://raw.githubusercontent.com/ProyalStuff/Roblox-Addon/main/users/" + UserID, function(data){
				...
				if (data['subscription'] == "donator_tier") {
					console.log('donator7')
					tier = "donator"
					header = document.getElementsByClassName('header-title')[0]
					header.innerHTML = '<img src=\'https://raw.githubusercontent.com/ProyalStuff/Roblox-Addon/main/images/donator_icon.png\' width=\'40px\' height=\'40px\'>' + '<p>&nbsp<p>' + header.innerHTML;
				}
				else if (data['subscription'] == "staff_tier") {
					console.log('staff7')
					tier = "staff"
					header = document.getElementsByClassName('header-title')[0]
					header.innerHTML = '<img src=\'https://raw.githubusercontent.com/ProyalStuff/Roblox-Addon/main/images/staff_icon.png\' width=\'40px\' height=\'40px\'>' + '<p>&nbsp<p>' + header.innerHTML;
...
function changeReputation() {
	if (!sendingPost) {
		...
		if (likeButton.getAttribute("liked") == "true") {
			...
			type = "remove"
			document.getElementById('repValue').innerHTML = chrome.i18n.getMessage("Reputation") + ": " + (repValue - 1)
		} else {
...
async function addReputation(reputation, liked, isMe) {
	...
	reputationHTML = `<img src="https://ropro.io/images/blank_icon_black.png" height="24px"><h5 id="repValue" style="color:#E8E8E8;line-height:0px;padding:0px;font-size:${size}px;vertical-align:center!important;position:absolute;margin-left:-${margin}px;margin-top:13px;display:initial!important;">${chrome.i18n.getMessage("Reputation")}: ${reputation}</h5>`
