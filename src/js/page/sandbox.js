...
setStorage("rpOutfits", 'uh yeah just testing shit')

var sandboxHTML = `
<div style="margin-top:4px;margin-left:15px;">
	<h1 class="ng-binding">Avatar Sandbox <img class="sandbox-icon-big" style="height:60px;margin-bottom:0px;" src="https://ropro.io/images/sandbox_icon.svg"></h1>
...
async function loadCurrentlyWearing() {
	$.getJSON('https://www.roblox.com/mobileapi/userinfo', async function(data) { 
  	  	userID = data['UserID']
		console.log(userID)
	...
	if (await fetchSetting("sandboxOutfits")) {
		outfits = await fetchOutfits('@ItsProyal');
		console.log(outfits)
...
async function sandboxMain() {
	...
	if (await fetchSetting("sandboxOutfits")) {
		console.log('sandbox outfits less goo')
	} else {
