...
async function loadTrades() {
	...
	if (tradesArray.length < 10 && cursor != null && currentlyLoading == false) {
...
async function getTrades() {
	...
	if (tradeRows.length > 0) {
		trades = []
		cachedTrades = await getStorage("cachedTrades")
		console.log(cachedTrades)
		...
		Promise.all(trades).then(async (values) => {
			for (i = 0; i < values.length; i++) {
				cachedTrades[values[i].id] = values[i]
			}
			setStorage("cachedTrades", cachedTrades)
			console.log(cachedTrades)
			trades = {data:[]}
			trades.data = values
