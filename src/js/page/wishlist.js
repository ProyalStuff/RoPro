...
var wishlistHTML = `<div class="container-header">
	<h3>RoPro Trade Offers <img class="offers-icon-big" style="height:15px;margin-bottom:0px;margin-left:5px;" src="https://ropro.io/images/offers_icon.svg"></h3>
	  <button id="createOfferButton" type="button" class="btn-fixed-width-lg btn-growth-lg" style="background-color:#0084dd;border:0px;font-size:18px;padding:5px;float:right;">${chrome.i18n.getMessage("CreateOffer")}</button>
   </div>
...
async function getWishes(wantID, wishPage) {
	...
	if (wishlist['valid'] == true) {
		if (wishlist['none'] == false) {
			...
			document.getElementById('wishlistOffers').innerHTML = '<p style="margin-left:20px;margin-bottom:30px;">' + chrome.i18n.getMessage('NoTradeOffersForThisItemBeTheFirstToPostOne') + '</p>'
		}
	} else {
